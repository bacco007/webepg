import type { TimelineProvider } from "../timeline-data";

export const transact: TimelineProvider = {
  category: "Subscription",
  country: "Australia",
  data: {
    axis: {
      end: 2017,
      start: 2002,
      unit: "year",
    },
    channels: {
      "1": [
        {
          channel_genre: "Information",
          channel_name: "TransACT",
          from: 2002,
          to: 2017.4,
        },
      ],
      "2": [
        {
          channel_genre: "FTA",
          channel_name: "ABC Canberra",
          from: 2002,
          to: 2017.4,
        },
      ],
      "3": [
        {
          channel_genre: "FTA",
          channel_name: "ABC Kids",
          from: 2002,
          to: 2003,
        },
        { channel_genre: "FTA", channel_name: "SBS", from: 2004.4, to: 2017.4 },
      ],
      "4": [
        {
          channel_genre: "Information",
          channel_name: "TransACT Promo Channel",
          from: 2002,
          to: 2017.4,
        },
      ],
      "5": [
        {
          channel_genre: "FTA",
          channel_name: "ABC Fly TV",
          from: 2002,
          to: 2003,
        },
        {
          channel_genre: "Kids",
          channel_name: "Cartoon Network",
          from: 2004.4,
          to: 2008,
        },
        {
          channel_genre: "FTA",
          channel_name: "Southern Cross Ten",
          from: 2008,
          to: 2017.4,
        },
      ],
      "6": [
        {
          channel_genre: "Kids",
          channel_name: "The Disney Channel",
          from: 2002,
          to: 2008,
        },
        {
          channel_genre: "FTA",
          channel_name: "Prime7",
          from: 2008,
          to: 2017.4,
        },
      ],
      "7": [
        {
          channel_genre: "FTA",
          channel_name: "Prime Canberra",
          from: 2002,
          to: 2008,
        },
      ],
      "8": [
        { channel_genre: "FTA", channel_name: "SBS", from: 2002, to: 2004.4 },
        {
          channel_genre: "FTA",
          channel_name: "WIN Television",
          from: 2008,
          to: 2017.4,
        },
      ],
      "9": [
        {
          channel_genre: "FTA",
          channel_name: "WIN Canberra",
          from: 2002,
          to: 2008,
        },
      ],
      "10": [
        {
          channel_genre: "FTA",
          channel_name: "Southern Cross TEN",
          from: 2002,
          to: 2008,
        },
      ],
      "11": [
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - House of Representatives",
          from: 2002,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "CNN International",
          from: 2004.4,
          to: 2008,
        },
      ],
      "12": [
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - Senate",
          from: 2002,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 2004.4,
          to: 2008,
        },
      ],
      "13": [
        {
          channel_genre: "Extra",
          channel_name: "Foxtel on TransACT EPG",
          from: 2002.12,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "CNBC Australia",
          from: 2004.4,
          to: 2008,
        },
      ],
      "14": [
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 2002,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "Bloomberg Television",
          from: 2004.4,
          to: 2008,
        },
      ],
      "15": [
        {
          channel_genre: "Movies",
          channel_name: "Turner Classic Movies",
          from: 2004.4,
          to: 2008,
        },
      ],
      "16": [
        { channel_genre: "News", channel_name: "CNBC", from: 2002, to: 2004.4 },
        {
          channel_genre: "International",
          channel_name: "Deutsche Welle (German)",
          from: 2004.4,
          to: 2008,
        },
      ],
      "17": [
        {
          channel_genre: "FTA",
          channel_name: "SBS World News",
          from: 2004.4,
          to: 2017.4,
        },
      ],
      "18": [
        {
          channel_genre: "International",
          channel_name: "Liaoning TV (Chinese)",
          from: 2002,
          to: 2008,
        },
      ],
      "19": [
        {
          channel_genre: "International",
          channel_name: "TV5Monde (French)",
          from: 2002,
          to: 2008,
        },
      ],
      "20": [
        {
          channel_genre: "International",
          channel_name: "Deutsche Welle (German)",
          from: 2002,
          to: 2004.4,
        },
      ],
      "22": [
        {
          channel_genre: "FTA",
          channel_name: "ABC Analogue",
          from: 2004.4,
          to: 2005,
        },
        { channel_genre: "FTA", channel_name: "ABC2", from: 2005, to: 2017.4 },
      ],
      "23": [
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          from: 2004.5,
          to: 2008,
        },
        { channel_genre: "FTA", channel_name: "ABC3", from: 2010, to: 2017.4 },
      ],
      "24": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel",
          from: 2005,
          to: 2006,
        },
        {
          channel_genre: "FTA",
          channel_name: "ABC News 24 HD",
          from: 2011,
          to: 2017.4,
        },
      ],
      "25": [
        {
          channel_genre: "News",
          channel_name: "Channel NewsAsia",
          from: 2005.1,
          to: 2008,
        },
      ],
      "26": [
        {
          channel_genre: "Kids",
          channel_name: "Boomerang",
          from: 2004.5,
          to: 2008,
        },
      ],
      "27": [
        {
          channel_genre: "FTA",
          channel_name: "Prime Analogue",
          from: 2004.4,
          to: 2005,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Real Time",
          from: 2005,
          to: 2008,
        },
      ],
      "28": [
        {
          channel_genre: "Documentary",
          channel_name: "Animal Planet",
          from: 2004.5,
          to: 2008,
        },
      ],
      "29": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Travel and Living",
          from: 2004.5,
          to: 2008,
        },
      ],
      "30": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Science",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "FTA",
          channel_name: "SBS One HD",
          from: 2008,
          to: 2017.4,
        },
      ],
      "31": [
        {
          channel_genre: "FTA",
          channel_name: "SBS World News Channel",
          from: 2008,
          to: 2017.4,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Home and Health",
          from: 2004.5,
          to: 2008,
        },
      ],
      "32": [
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic Channel",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "FTA",
          channel_name: "SBS Two",
          from: 2009,
          to: 2017.4,
        },
      ],
      "33": [
        {
          channel_genre: "Documentary",
          channel_name: "Adventure One",
          from: 2004.5,
          to: 2008,
        },
      ],
      "38": [
        {
          channel_genre: "Entertainment",
          channel_name: "Fashion TV",
          from: 2004.5,
          to: 2008,
        },
      ],
      "39": [
        {
          channel_genre: "Entertainment",
          channel_name: "E!",
          from: 2004.5,
          to: 2008,
        },
      ],
      "41": [
        {
          channel_genre: "News",
          channel_name: "CNN International",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
        {
          channel_genre: "Religious",
          channel_name: "EWTN",
          from: 2004.5,
          to: 2017.4,
        },
      ],
      "42": [
        {
          channel_genre: "News",
          channel_name: "CNNfn",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
        {
          channel_genre: "Religious",
          channel_name: "TBN",
          from: 2004.12,
          to: 2005,
        },
        {
          channel_genre: "Religious",
          channel_name: "Australian Christian Channel",
          from: 2005,
          to: 2008,
        },
      ],
      "43": [
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - Senate Estimates",
          from: 2004.4,
          to: 2017.4,
        },
      ],
      "44": [
        {
          channel_genre: "Movies",
          channel_name: "Turner Classic Movies",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - House of Representatives",
          from: 2004.4,
          to: 2017.4,
        },
      ],
      "45": [
        {
          channel_genre: "Music",
          channel_name: "Soundtrack Channel",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - Senate",
          from: 2004.4,
          to: 2017.4,
        },
      ],
      "46": [
        {
          channel_genre: "Kids",
          channel_name: "Cartoon Network",
          channel_notes: "Part of Primestar Trial Service",
          from: 2002.2,
          to: 2004.4,
        },
      ],
      // —— Existing 2011+ entries that remain unchanged ——
      "50": [
        {
          channel_genre: "FTA",
          channel_name: "One HD",
          from: 2008,
          to: 2017.4,
        },
      ],
      "55": [
        {
          channel_genre: "FTA",
          channel_name: "Eleven",
          from: 2011,
          to: 2017.4,
        },
      ],
      "62": [
        { channel_genre: "FTA", channel_name: "7Two", from: 2010, to: 2017.4 },
      ],
      "63": [
        {
          channel_genre: "FTA",
          channel_name: "7mate HD",
          from: 2011,
          to: 2017.4,
        },
      ],
      "80": [
        {
          channel_genre: "FTA",
          channel_name: "GEM HD",
          from: 2011,
          to: 2017.4,
        },
      ],
      "88": [
        { channel_genre: "FTA", channel_name: "GO!", from: 2009, to: 2017.4 },
      ],
      "201": [
        {
          channel_genre: "Kids",
          channel_name: "Disney Channel",
          from: 2008,
          to: 2017.4,
        },
      ],
      "202": [
        {
          channel_genre: "Kids",
          channel_name: "Playhouse Disney",
          from: 2007.11,
          to: 2017.4,
        },
      ],
      "203": [
        {
          channel_genre: "Kids",
          channel_name: "Cartoon Network",
          from: 2008,
          to: 2017.4,
        },
      ],
      "204": [
        {
          channel_genre: "Kids",
          channel_name: "Boomerang",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Kids",
          channel_name: "Boomerang",
          from: 2008,
          to: 2017.4,
        },
      ],
      "251": [
        { channel_genre: "Music", channel_name: "MTV", from: 2008, to: 2017.4 },
      ],
      "252": [
        { channel_genre: "Music", channel_name: "VH1", from: 2008, to: 2011 },
        {
          channel_genre: "Music",
          channel_name: "MTV Classic",
          from: 2011,
          to: 2017.4,
        },
      ],
      "253": [
        {
          channel_genre: "Entertainment",
          channel_name: "E!",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "E!",
          from: 2008,
          to: 2017.4,
        },
      ],
      "254": [
        {
          channel_genre: "Entertainment",
          channel_name: "Fashion TV",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "Fashion TV",
          from: 2008,
          to: 2017.4,
        },
      ],
      "255": [
        {
          channel_genre: "Music",
          channel_name: "MCM TOP",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Music",
          channel_name: "MCM TOP",
          from: 2008,
          to: 2013,
        },
      ],
      "301": [
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic Channel",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic Channel",
          from: 2008,
          to: 2017.4,
        },
      ],
      "302": [
        {
          channel_genre: "Documentary",
          channel_name: "Nat Geo Adventure",
          from: 2008,
          to: 2014,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Nat Geo People",
          from: 2014,
          to: 2017.4,
        },
      ],
      "306": [
        {
          channel_genre: "Documentary",
          channel_name: "Animal Planet",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Animal Planet",
          from: 2008,
          to: 2017.4,
        },
      ],
      "307": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Turbo",
          from: 2008,
          to: 2017.4,
        },
      ],
      "308": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Travel and Living",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Travel and Living",
          from: 2008,
          to: 2011,
        },
        {
          channel_genre: "Documentary",
          channel_name: "TLC",
          from: 2011,
          to: 2017.4,
        },
      ],
      "309": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Home and Health",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Home and Health",
          from: 2008,
          to: 2017.4,
        },
      ],
      "310": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Science",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Science",
          from: 2008,
          to: 2017.4,
        },
      ],
      "351": [
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          from: 2008,
          to: 2017.4,
        },
      ],
      "354": [
        {
          channel_genre: "Sports",
          channel_name: "Eurosport",
          from: 2008,
          to: 2017.4,
        },
      ],
      "355": [
        {
          channel_genre: "Sports",
          channel_name: "Eurosportnews",
          from: 2008,
          to: 2017.4,
        },
      ],
      "356": [
        {
          channel_genre: "Sports",
          channel_name: "Setanta Sports",
          from: 2009,
          to: 2014,
        },
        {
          channel_genre: "Sports",
          channel_name: "beIN Sports",
          from: 2014,
          to: 2017.4,
        },
      ],
      "411": [
        {
          channel_genre: "Movies",
          channel_name: "Movie One",
          from: 2008.6,
          to: 2012.12,
        },
      ],
      "412": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Two",
          from: 2008.6,
          to: 2012.12,
        },
      ],
      "413": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Extra",
          from: 2008.6,
          to: 2012.12,
        },
      ],
      "415": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Greats",
          from: 2008.6,
          to: 2012.12,
        },
      ],
      "417": [
        {
          channel_genre: "Movies",
          channel_name: "Turner Classic Movies",
          from: 2008,
          to: 2017.4,
        },
      ],
      "451": [
        {
          channel_genre: "News",
          channel_name: "CNN International",
          from: 2008,
          to: 2017.4,
        },
      ],
      "452": [
        {
          channel_genre: "News",
          channel_name: "BBC World News",
          from: 2008,
          to: 2017.4,
        },
      ],
      "453": [
        {
          channel_genre: "News",
          channel_name: "CNBC Australia",
          from: 2008,
          to: 2017.4,
        },
      ],
      "454": [
        {
          channel_genre: "News",
          channel_name: "Bloomberg Television",
          from: 2008,
          to: 2017.4,
        },
      ],
      "455": [
        {
          channel_genre: "News",
          channel_name: "Channel NewsAsia",
          from: 2008,
          to: 2017.4,
        },
      ],
      "458": [
        {
          channel_genre: "News",
          channel_name: "euronews",
          from: 2008,
          to: 2017.4,
        },
      ],
      "459": [
        {
          channel_genre: "News",
          channel_name: "CCTV9",
          from: 2008,
          to: 2017.4,
        },
      ],
      "460": [
        {
          channel_genre: "News",
          channel_name: "Al Jazeera English",
          from: 2008,
          to: 2017.4,
        },
      ],
      "461": [
        {
          channel_genre: "News",
          channel_name: "euronews (Italian)",
          from: 2008,
          to: 2017.4,
        },
      ],
      "462": [
        {
          channel_genre: "News",
          channel_name: "euronews (Spanish)",
          from: 2008,
          to: 2017.4,
        },
      ],
      "463": [
        {
          channel_genre: "News",
          channel_name: "CCTV4",
          from: 2008,
          to: 2017.4,
        },
      ],
      "501": [
        {
          channel_genre: "Other",
          channel_name: "Channelvision",
          from: 2008,
          to: 2017.4,
        },
      ],
      "502": [
        {
          channel_genre: "Other",
          channel_name: "NITV",
          from: 2009,
          to: 2017.4,
        },
      ],
      "510": [
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - House of Representatives",
          from: 2008,
          to: 2017.4,
        },
      ],
      "511": [
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - Senate",
          from: 2008,
          to: 2017.4,
        },
      ],
      "512": [
        {
          channel_genre: "News",
          channel_name: "Federal Parliament - Parliamentary Committees",
          from: 2008,
          to: 2017.4,
        },
      ],
      "551": [
        {
          channel_genre: "Religious",
          channel_name: "EWTN",
          from: 2004.5,
          to: 2008,
        },
        {
          channel_genre: "Religious",
          channel_name: "EWTN",
          from: 2008,
          to: 2017.4,
        },
      ],
      "552": [
        {
          channel_genre: "Religious",
          channel_name: "Australian Christian Channel",
          from: 2008,
          to: 2017.4,
        },
      ],
      "601": [
        {
          channel_genre: "International",
          channel_name: "ERT World (Greek)",
          from: 2008,
          to: 2017.4,
        },
      ],
      "602": [
        {
          channel_genre: "International",
          channel_name: "RTPi (Portuguese)",
          from: 2008,
          to: 2017.4,
        },
      ],
      "603": [
        {
          channel_genre: "International",
          channel_name: "TV5Monde (French)",
          from: 2008,
          to: 2017.4,
        },
      ],
      "604": [
        {
          channel_genre: "International",
          channel_name: "Deutsche Welle (German)",
          from: 2008,
          to: 2017.4,
        },
      ],
      F1: [
        {
          channel_genre: "Entertainment",
          channel_name: "Arena",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F2: [
        {
          channel_genre: "News",
          channel_name: "BBC World",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F3: [
        {
          channel_genre: "News",
          channel_name: "Bloomberg TV",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F4: [
        {
          channel_genre: "Music",
          channel_name: "Channel [V]",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F5: [
        {
          channel_genre: "News",
          channel_name: "CNBC",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F6: [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Channel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F7: [
        {
          channel_genre: "Entertainment",
          channel_name: "Encore",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F8: [
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F9: [
        {
          channel_genre: "Entertainment",
          channel_name: "fashionTV",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F10: [
        {
          channel_genre: "Sports",
          channel_name: "FOX Footy Channel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F11: [
        {
          channel_genre: "Sports",
          channel_name: "FOX Footy Extra",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F12: [
        {
          channel_genre: "Entertainment",
          channel_name: "FOX Classics/Fox Kids",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F13: [
        {
          channel_genre: "News",
          channel_name: "FOX News",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F14: [
        {
          channel_genre: "Sports",
          channel_name: "FOX Sport",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F15: [
        {
          channel_genre: "Sports",
          channel_name: "FOX Sport 2",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F16: [
        {
          channel_genre: "Entertainment",
          channel_name: "FOX8",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F17: [
        {
          channel_genre: "Other",
          channel_name: "FOXTEL - EPG",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F18: [
        {
          channel_genre: "Movies",
          channel_name: "Hallmark",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F19: [
        {
          channel_genre: "Documentary",
          channel_name: "The History Channel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F20: [
        {
          channel_genre: "Entertainment",
          channel_name: "Lifestyle Channel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F21: [
        {
          channel_genre: "Movies",
          channel_name: "MOVIE EXTRA",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F22: [
        {
          channel_genre: "Movies",
          channel_name: "MOVIE GREATS",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F23: [
        {
          channel_genre: "Movies",
          channel_name: "MOVIE ONE",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F24: [
        {
          channel_genre: "Music",
          channel_name: "Music Max",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F25: [
        {
          channel_genre: "Music",
          channel_name: "MTV",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F26: [
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F27: [
        {
          channel_genre: "Kids",
          channel_name: "Nickelodeon",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F28: [
        {
          channel_genre: "Entertainment",
          channel_name: "Ovation",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F29: [
        {
          channel_genre: "Entertainment",
          channel_name: "Showtime",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F30: [
        {
          channel_genre: "Entertainment",
          channel_name: "Showtime 2",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F31: [
        {
          channel_genre: "Sports",
          channel_name: "Sky Racing",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F32: [
        {
          channel_genre: "News",
          channel_name: "Sky News Australia",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F33: [
        {
          channel_genre: "Entertainment",
          channel_name: "thecomedychannel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F34: [
        {
          channel_genre: "Entertainment",
          channel_name: "TV1",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F35: [
        {
          channel_genre: "Other",
          channel_name: "TVSN",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F36: [
        {
          channel_genre: "Entertainment",
          channel_name: "UKTV",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F37: [
        {
          channel_genre: "Other",
          channel_name: "The Weather Channel",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
      F38: [
        {
          channel_genre: "Entertainment",
          channel_name: "fx",
          channel_notes:
            "Part of Foxtel on TransACT Service, available via TransTV Menu",
          from: 2002.12,
          to: 2004.4,
        },
      ],
    },
    description: "Timeline of channel changes from 1995 to 1998",
    events: [
      {
        label: "Foxtel on TransACT Commences",
        note: "Foxtel on TransACT Commences",
        type: "Launch",
        when: "2002.12",
      },
      {
        label: "Primestar Trial Service Commences",
        note: "Primestar Trial Service Commences",
        type: "Launch",
        when: "2002.2",
      },
      {
        label: "Foxtel on TransACT Ceases",
        note: "Foxtel on TransACT Ceases",
        type: "Closure",
        when: "2004.4",
      },
      {
        label: "iiNet acquires TransACT",
        note: "iiNet acquires TransACT",
        type: "Launch",
        when: "2011.11",
      },
      {
        label: "Transition to Fetch TV",
        note: "Transition to Fetch TV",
        type: "Closure",
        when: "2017.4",
      },
    ],
    title: "TransACT TransTV Channel History (2001-2017)",
  },
  description: "TransACT's TransTV Service from 2001 to 2017",
  id: "transact",
  name: "TransACT TransTV",
};
