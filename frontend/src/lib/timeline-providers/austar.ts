import type { TimelineProvider } from "../timeline-data";

export const austar: TimelineProvider = {
  category: "Subscription",
  country: "Australia",
  data: {
    axis: {
      end: 2007,
      start: 1995,
      unit: "year",
    },
    channels: {
      "1": [
        {
          channel_genre: "Sports",
          channel_name: "Premier Sports Network",
          from: 1995.8,
          to: 1996.3,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports",
          from: 1996.3,
          to: 1998,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "TV1",
          from: 1998,
          to: 2007,
        },
      ],
      "2": [
        {
          channel_genre: "Movies",
          channel_name: "Showtime",
          from: 1995.8,
          to: "2001.10",
        },
        {
          channel_genre: "FTA",
          channel_name: "ABC Television",
          from: "2001.10",
          to: 2007,
        },
      ],
      "3": [
        {
          channel_genre: "Movies",
          channel_name: "Encore",
          from: 1995.8,
          to: "2001.10",
        },
        {
          channel_genre: "Entertainment",
          channel_name: "Arena",
          from: "2001.10",
          to: 2007,
        },
      ],
      "4": [
        {
          channel_genre: "Entertainment",
          channel_name: "TV1",
          from: 1995.8,
          to: 1998,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports",
          from: 1998,
          to: "2001.10",
        },
        {
          channel_genre: "Lifestyle",
          channel_name: "The LifeStyle Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "5": [
        {
          channel_genre: "Entertainment",
          channel_name: "Arena",
          from: 1995.8,
          to: "2001.10",
        },
        {
          channel_genre: "Entertainment",
          channel_name: "The Comedy Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "6": [
        {
          channel_genre: "Music",
          channel_name: "Red",
          from: 1995.8,
          to: 1997.4,
        },
        {
          channel_genre: "Music",
          channel_name: "Channel [V]",
          from: 1997.4,
          to: "2001.10",
        },
        {
          channel_genre: "Entertainment",
          channel_name: "fx",
          from: "2001.10",
          to: 2003,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "W",
          from: 2003,
          to: 2007,
        },
      ],
      "7": [
        {
          channel_genre: "Shared",
          channel_name: "Nickelodeon & Nick @ Nite",
          from: 1995.8,
          to: 2000,
        },
        {
          channel_genre: "Kids",
          channel_name: "Nickelodeon",
          from: 2000,
          to: "2001.10",
        },
        {
          channel_genre: "Entertainment",
          channel_name: "UK.TV",
          from: "2001.10",
          to: 2007,
        },
      ],
      "8": [
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Channel",
          from: 1995.8,
          to: "2001.10",
        },
        {
          channel_genre: "Entertainment",
          channel_name: "FOX8",
          from: "2001.10",
          to: 2007,
        },
      ],
      "9": [
        {
          channel_genre: "News",
          channel_name: "Asia Business News",
          from: 1995.8,
          to: 1997,
        },
        {
          channel_genre: "News",
          channel_name: "CNBC Asia",
          from: 1997,
          to: 1998,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports Two",
          from: 1998,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "The Games Channel",
          from: 2003,
          to: 2003,
        },
      ],
      "10": [
        {
          channel_genre: "Shopping",
          channel_name: "The Value Channel",
          from: 1995.12,
          to: 1996.6,
        },
        {
          channel_genre: "Shopping",
          channel_name: "TVSN",
          from: 1996.6,
          to: 1998,
        },
        {
          channel_genre: "Lifestyle",
          channel_name: "The Lifestyle Channel",
          from: 1998,
          to: "2001.10",
        },
        {
          channel_genre: "Documentary",
          channel_name: "Discovery Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "11": [
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 1995.8,
          to: 1998,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "The Comedy Channel",
          from: 1998,
          to: "2001.10",
        },
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "12": [
        {
          channel_genre: "Entertainment",
          channel_name: "The Comedy Channel",
          from: 1996,
          to: 1998,
        },
        {
          channel_genre: "Documentary",
          channel_name: "National Geographic Channel",
          from: 1998,
          to: "2001.10",
        },
        {
          channel_genre: "Documentary",
          channel_name: "The History Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "13": [
        {
          channel_genre: "Shared",
          channel_name: "Preview Channel & Nightmoves",
          from: 1995.8,
          to: 1998,
        },
        {
          channel_genre: "PPV",
          channel_name: "Main Attraction",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "PPV",
          channel_name: "Main Event & Adults Only",
          from: 1999.5,
          to: "2001.10",
        },
        {
          channel_genre: "FTA",
          channel_name: "SBS Television",
          from: 2003,
          to: 2007,
        },
      ],
      "14": [
        {
          channel_genre: "Music",
          channel_name: "Country Music Television",
          from: 1995.8,
          to: 2000,
        },
        {
          channel_genre: "Music",
          channel_name: "MusicCountry",
          from: 2000,
          to: "2001.10",
        },
        {
          channel_genre: "FTA",
          channel_name: "ABC Kids/Fly TV",
          from: "2001.10",
          to: 2003.7,
        },
      ],
      "15": [
        {
          channel_genre: "Shared",
          channel_name: "Cartoon Network & TNT",
          from: 1995.8,
          to: 1998,
        },
        {
          channel_genre: "Shared",
          channel_name: "Sky Racing & Adults Only",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "Sports",
          channel_name: "Sky Racing",
          from: 1999.5,
          to: "2001.10",
        },
        {
          channel_genre: "Kids",
          channel_name: "Nickelodeon",
          from: "2001.10",
          to: 2007,
        },
      ],
      "16": [
        {
          channel_genre: "Lifestyle",
          channel_name: "The LifeStyle Channel",
          from: 1997,
          to: 1998,
        },
        {
          channel_genre: "Kids",
          channel_name: "The Disney Channel",
          from: 1999.1,
          to: 2007,
        },
      ],
      "17": [
        {
          channel_genre: "Movies",
          channel_name: "Movie One",
          from: 1999.1,
          to: "2001.10",
        },
        {
          channel_genre: "Kids",
          channel_name: "Cartoon Network",
          from: "2001.10",
          to: 2007,
        },
      ],
      "18": [
        {
          channel_genre: "Movies",
          channel_name: "World Movies",
          from: 1995.8,
          to: 1998,
        },
        {
          channel_genre: "Movies",
          channel_name: "Movie Extra",
          from: 1999.1,
          to: "2001.10",
        },
        {
          channel_genre: "Shopping",
          channel_name: "TVSN: Television Shopping Network",
          from: "2001.10",
          to: 2007,
        },
      ],
      "19": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Greats",
          from: 1999.1,
          to: 2001.1,
        },
        {
          channel_genre: "Sports",
          channel_name: "C7 Sport Blue",
          from: 2001.1,
          to: 2002.4,
        },
        {
          channel_genre: "Shopping",
          channel_name: "Expo",
          from: 2002.4,
          to: 2007,
        },
      ],
      "20": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports Two",
          from: 1997,
          to: 1998,
        },
        {
          channel_genre: "Shared",
          channel_name: "Cartoon Network & TNT",
          from: 1998,
          to: 1999.4,
        },
        {
          channel_genre: "Sports",
          channel_name: "C7 Sport",
          from: 1999.4,
          to: 2001,
        },
        {
          channel_genre: "Sports",
          channel_name: "C7 Sport Gold",
          from: 2001,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports 1",
          from: "2001.10",
          to: 2007,
        },
      ],
      "21": [
        {
          channel_genre: "PPV",
          channel_name: "Pay Per View",
          from: 1995.8,
          to: 1997,
        },
        {
          channel_genre: "News",
          channel_name: "Weather 21",
          from: 1999.1,
          to: 1999.3,
        },
        {
          channel_genre: "Shared",
          channel_name: "Cartoon Network & TNT",
          from: 1999.3,
          to: 2000,
        },
        {
          channel_genre: "News",
          channel_name: "The Weather Channel",
          from: 2000,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Sports 2",
          from: "2001.10",
          to: 2007,
        },
      ],
      "22": [
        {
          channel_genre: "News",
          channel_name: "CNBC Asia",
          from: 1998,
          to: 1999.3,
        },
        {
          channel_genre: "News",
          channel_name: "Weather 21",
          from: 1999.3,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "Sky Racing",
          from: "2001.10",
          to: 2007,
        },
      ],
      "23": [
        {
          channel_genre: "Movies",
          channel_name: "World Movies",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "News",
          channel_name: "CNBC Asia",
          from: 1999.5,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "C7 Sport Gold",
          from: "2001.10",
          to: 2002.4,
        },
      ],
      "24": [
        {
          channel_genre: "Shopping",
          channel_name: "TVSN",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "Movies",
          channel_name: "World Movies",
          from: 1999.5,
          to: "2001.10",
        },
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          from: "2001.10",
          to: 2007,
        },
      ],
      "25": [
        {
          channel_genre: "News",
          channel_name: "CNN International",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "Shopping",
          channel_name: "TVSN",
          from: 1999.5,
          to: "2001.10",
        },
        {
          channel_genre: "PPV",
          channel_name: "MainEvent / Fashion TV / Adults only",
          from: "2001.10",
          to: 2007,
        },
      ],
      "26": [
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 1998,
          to: 1999.5,
        },
        {
          channel_genre: "News",
          channel_name: "CNN International",
          from: 1999.5,
          to: "2001.10",
        },
      ],
      "27": [
        {
          channel_genre: "Sports",
          channel_name: "ESPN",
          from: 1999,
          to: "2001.10",
        },
        {
          channel_genre: "Music",
          channel_name: "Channel [V]",
          from: "2001.10",
          to: 2007,
        },
      ],
      "28": [
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 1999.4,
          to: "2000.4",
        },
        {
          channel_genre: "Music",
          channel_name: "musicMAX",
          from: "2001.3",
          to: 2004,
        },
        {
          channel_genre: "Music",
          channel_name: "Max",
          from: 2004,
          to: 2007,
        },
      ],
      "29": [
        {
          channel_genre: "Entertainment",
          channel_name: "FOX8",
          from: 1999.4,
          to: "2001.10",
        },
        {
          channel_genre: "Music",
          channel_name: "MC (Music Country)",
          from: "2001.10",
          to: 2002,
        },
        {
          channel_genre: "Music",
          channel_name: "CMC",
          from: 2002,
          to: 2007,
        },
      ],
      "30": [
        {
          channel_genre: "Entertainment",
          channel_name: "UK.TV",
          from: 1999.4,
          to: "2001.10",
        },
        {
          channel_genre: "News",
          channel_name: "Sky News Australia",
          from: "2001.10",
          to: 2007,
        },
      ],
      "31": [
        {
          channel_genre: "Entertainment",
          channel_name: "Hallmark Channel",
          from: 1999.4,
          to: "2001.10",
        },
        {
          channel_genre: "News",
          channel_name: "The Weather Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "32": [
        {
          channel_genre: "Shared",
          channel_name: "fX & fX Movies",
          from: 1999.4,
          to: 2000,
        },
        {
          channel_genre: "Entertainment",
          channel_name: "fX",
          from: 2000,
          to: "2001.10",
        },
        {
          channel_genre: "News",
          channel_name: "CNN International",
          from: "2001.10",
          to: 2007,
        },
      ],
      "33": [
        {
          channel_genre: "Kids",
          channel_name: "The Disney Channel",
          from: 2000,
          to: "2001.10",
        },
        {
          channel_genre: "News",
          channel_name: "CNBC Asia",
          from: "2001.10",
          to: 2007,
        },
      ],
      "34": [
        {
          channel_genre: "News",
          channel_name: "Sky News Australia",
          from: 2000.4,
          to: "2001.3",
        },
        {
          channel_genre: "News",
          channel_name: "Bloomberg",
          from: "2001.3",
          to: 2007,
        },
      ],
      "35": [
        {
          channel_genre: "Sports",
          channel_name: "C7 Olympics",
          channel_notes: "Popup Channel for 2000 Sydney Olympics",
          from: "2000.9",
          to: "2000.11",
        },
        {
          channel_genre: "News",
          channel_name: "BBC World",
          from: 2003,
          to: 2007,
        },
      ],
      "36": [
        {
          channel_genre: "Sports",
          channel_name: "C7 Games",
          channel_notes: "Popup Channel for 2000 Sydney Olympics",
          from: "2000.9",
          to: "2000.11",
        },
        {
          channel_genre: "Movies",
          channel_name: "TCM",
          from: "2001.10",
          to: 2007,
        },
      ],
      "37": [
        {
          channel_genre: "Entertainment",
          channel_name: "Hallmark Channel",
          from: "2001.10",
          to: 2007,
        },
      ],
      "38": [
        {
          channel_genre: "Movies",
          channel_name: "Movie One",
          from: "2001.10",
          to: 2007,
        },
      ],
      "39": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Extra",
          from: "2001.10",
          to: 2007,
        },
      ],
      "40": [
        {
          channel_genre: "Movies",
          channel_name: "Movie Greats",
          from: "2001.10",
          to: 2007,
        },
      ],

      "41": [
        {
          channel_genre: "Movies",
          channel_name: "Showtime",
          from: "2001.10",
          to: 2007,
        },
      ],
      "43": [
        {
          channel_genre: "Movies",
          channel_name: "Encore",
          from: "2001.10",
          to: 2007,
        },
      ],
      "44": [
        {
          channel_genre: "Movies",
          channel_name: "World Movies",
          from: "2001.10",
          to: 2007,
        },
      ],
      "49": [
        {
          channel_genre: "FTA",
          channel_name: "iTV64 (Darwin Only)",
          from: 2001,
          to: 2007,
        },
      ],
      "50": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel (Qld)",
          from: 2002,
          to: 2003,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Extra (Qld)",
          from: 2003,
          to: 2007,
        },
      ],

      "51": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel (NSW)",
          from: 2002,
          to: 2003,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Extra (NSW)",
          from: 2003,
          to: 2007,
        },
      ],
      "52": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel (Vic/Tas)",
          from: 2002,
          to: 2003,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Extra (Vic/Tas)",
          from: 2003,
          to: 2007,
        },
      ],
      "53": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel (SA/NT)",
          from: 2002,
          to: 2003,
        },
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Extra (SA/NT)",
          from: 2003,
          to: 2007,
        },
      ],
      "54": [
        {
          channel_genre: "Sports",
          channel_name: "Fox Footy Channel",
          from: 2003,
          to: 2007,
        },
      ],
      "90": [
        {
          channel_genre: "FTA",
          channel_name: "SBS (Darwin Only)",
          from: 2001,
          to: 2007,
        },
      ],
      "91": [
        {
          channel_genre: "Information",
          channel_name: "INFO",
          from: "2001.10",
          to: 2007,
        },
      ],
      "97": [
        {
          channel_genre: "FTA",
          channel_name: "Seven Darwin (Darwin Only)",
          from: 2001,
          to: 2007,
        },
      ],
      "98": [
        {
          channel_genre: "FTA",
          channel_name: "Nine Darwin (Darwin Only)",
          from: 2001,
          to: 2007,
        },
        {
          channel_genre: "Information",
          channel_name:
            "Replacement for Pace Decoder - Installation Information",
          from: 2003,
          to: 2003,
        },
      ],
      "99": [
        {
          channel_genre: "Information",
          channel_name: "FYI: For Your Information",
          from: 2002,
          to: 2007,
        },
      ],
      R1: [
        {
          channel_genre: "Radio",
          channel_name: "BBC World Service",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Light Classical",
          from: 2001,
          to: 2007,
        },
      ],
      R2: [
        {
          channel_genre: "Radio",
          channel_name: "Voice Of America",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "New Age",
          from: 2001,
          to: 2007,
        },
      ],
      R3: [
        {
          channel_genre: "Radio",
          channel_name: "3UZ National Racing",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Radio Italia",
          from: 2001,
          to: 2007,
        },
      ],
      R4: [
        {
          channel_genre: "Radio",
          channel_name: "Sport 927",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "SPORT 927",
          from: 2002,
          to: 2007,
        },
      ],
      R5: [
        {
          channel_genre: "Radio",
          channel_name: "Radio News Extra",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "News and More",
          from: 2002,
          to: 2007,
        },
      ],
      R6: [
        {
          channel_genre: "Radio",
          channel_name: "Easy Listening",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "High Energy",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Rhythm Digital",
          from: 2002,
          to: 2007,
        },
      ],
      R7: [
        {
          channel_genre: "Radio",
          channel_name: "Top 100",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Country",
          from: 2001,
          to: 2007,
        },
      ],
      R8: [
        {
          channel_genre: "Radio",
          channel_name: "Light Classical",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Top 100",
          from: 2001,
          to: 2007,
        },
      ],
      R9: [
        {
          channel_genre: "Radio",
          channel_name: "Jazz",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Cafe Jazz",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Smooth Cafe",
          from: 2002,
          to: 2007,
        },
      ],
      R10: [
        {
          channel_genre: "Radio",
          channel_name: "Classic Rock",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Easy Listening",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Beautiful Moods",
          from: 2002,
          to: 2007,
        },
      ],
      R11: [
        {
          channel_genre: "Radio",
          channel_name: "Country Music",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Classic Hits",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Classic Gold",
          from: 2002,
          to: 2007,
        },
      ],
      R12: [
        {
          channel_genre: "Radio",
          channel_name: "Dance Music",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Special Occasion",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Special Events",
          from: 2002,
          to: 2007,
        },
      ],
      R13: [
        {
          channel_genre: "Radio",
          channel_name: "Environmental",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "AUSTAR Radio",
          from: 2001,
          to: 2007,
        },
      ],
      R14: [
        {
          channel_genre: "Radio",
          channel_name: "Test Transmission",
          from: 1995.8,
          to: 2001,
        },
        {
          channel_genre: "Radio",
          channel_name: "Retro Beat",
          from: 2001,
          to: 2007,
        },
      ],
      R15: [
        {
          channel_genre: "Radio",
          channel_name: "Underground Sounds",
          from: 2001,
          to: 2007,
        },
      ],
      R16: [
        {
          channel_genre: "Radio",
          channel_name: "Stage & Screen",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "JustRock",
          from: 2002,
          to: 2007,
        },
      ],
      R17: [
        {
          channel_genre: "Radio",
          channel_name: "Latin Heat",
          from: 2001,
          to: 2007,
        },
      ],
      R18: [
        {
          channel_genre: "Radio",
          channel_name: "Contemporary Blues",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Blues",
          from: 2002,
          to: 2007,
        },
      ],
      R19: [
        {
          channel_genre: "Radio",
          channel_name: "Soul Train",
          from: 2001,
          to: 2007,
        },
      ],
      R20: [
        {
          channel_genre: "Radio",
          channel_name: "Cocktail Lounge",
          from: 2001,
          to: 2007,
        },
      ],
      R21: [
        {
          channel_genre: "Radio",
          channel_name: "Hottest Hits",
          from: 2001,
          to: 2007,
        },
      ],
      R22: [
        {
          channel_genre: "Radio",
          channel_name: "Global Pulse",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Urban Pulse",
          from: 2002,
          to: 2007,
        },
      ],
      R23: [
        {
          channel_genre: "Radio",
          channel_name: "Contemporary Country",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Adult Contemporary",
          from: 2002,
          to: 2007,
        },
      ],
      R24: [
        {
          channel_genre: "Radio",
          channel_name: "Great Symphonies",
          from: 2001,
          to: 2002,
        },
        {
          channel_genre: "Radio",
          channel_name: "Eurobeat",
          from: 2002,
          to: 2007,
        },
      ],
    },
    description: "Austar's Analogue Service - 1995 to 2007",
    events: [
      {
        label: "Austar Launches",
        note: "Austar Launches",
        type: "Launch",
        when: 1995.8,
      },
      {
        label: "Austar acquires ECTV",
        note: "Austar acquires East Coast Television",
        type: "Launch",
        when: 1998.7,
      },
      {
        label: "Channel restructure",
        note: "Major channel renumbering",
        type: "Launch",
        when: "2001.10",
      },
      {
        label: "Austar Digital Launches",
        note: "Austar Digital Launches",
        type: "Launch",
        when: 2004.3,
      },
    ],
    title: "Freeview Australia History (2008-2024)",
  },
  description: "Austar's Analogue Service - 1995 to 2007",
  id: "austar",
  name: "Austar (Analogue)",
};
