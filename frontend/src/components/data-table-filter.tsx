...
export function DataTableFilter<TData, TValue>({
  ...
  return (
    <div className="flex justify-between items-start gap-2 w-full">
      <div className="flex md:flex-wrap flex-1 gap-2 w-full">
        <FilterSelector table={table} />
        <ActiveFilters table={table} />
      </div>
      <FilterActions table={table} />
...
export function ActiveFiltersMobileContainer({
  ...
  return (
    <div className="relative w-full overflow-x-hidden">
      {/* Left blur effect */}
      {showLeftBlur && (
        <div className="top-0 bottom-0 left-0 z-10 absolute bg-gradient-to-r from-background to-transparent w-16 animate-in pointer-events-none fade-in-0" />
      )}
      ...
      {/* Right blur effect */}
      {showRightBlur && (
        <div className="top-0 right-0 bottom-0 z-10 absolute bg-gradient-to-l from-background to-transparent w-16 animate-in pointer-events-none fade-in-0" />
      )}
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        align="start"
        side="bottom"
        className="p-0 w-fit origin-(--radix-popover-content-transform-origin)"
      >
        {content}
      </PopoverContent>
    </Popover>
  ...
  return (
    <CommandItem onSelect={() => setProperty(column.id)} className="group">
      <div className="flex justify-between items-center w-full">
        ...
        <ArrowRight className="opacity-0 group-aria-selected:opacity-100 size-4" />
      </div>
    </CommandItem>
  )
}
...
function renderFilter<TData, T extends ColumnDataType>(
  ...
  return (
    <div
      ...
      className="flex items-center bg-background shadow-xs border border-border rounded-2xl h-7 text-xs"
    >
      ...
      <Button
        variant="ghost"
        className="rounded-none rounded-r-2xl w-7 h-full text-xs"
...
export function FilterSubject<TData>({
  ...
  return (
    <span className="flex items-center gap-1 px-2 font-medium whitespace-nowrap select-none">
      {hasIcon && <meta.icon className="stroke-[2.25px] size-4" />}
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          className="m-0 p-0 px-2 rounded-none w-fit h-full text-xs whitespace-nowrap"
        >
          ...
          {options.map((v) => {
            ...
            return (
              <CommandItem
                ...
                className="group flex justify-between items-center gap-1.5"
              >
...
export function FilterValueNumberController<TData, TValue>({
  ...
  return (
    <Command>
      <CommandList className="px-2 py-2 w-[300px]">
        <CommandGroup>
            ...
            >
              ...
              <TabsContent value="range" className="flex flex-col gap-4 mt-4">
                ...
                <div className="gap-4 grid grid-cols-2">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-xs">Min</span>
                    <Input
                      type="number"
                      value={inputValues[0]}
                      onChange={(e) => handleInputChange(0, e.target.value)}
