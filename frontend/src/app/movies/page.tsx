...
function FilterSection({
  ...
  const totalAvailableOptions = useMemo(() => {
    return options.filter((option) => counts[option] > 0 || filters.includes(option)).length
  }, [options, counts, filters])
...
function MoviesPageContent() {
    ...
    uniqueCategories.forEach((category) => {
      counts[category] = moviesData.channels.filter(
        (ch) =>
          ...
          (selectedGroups.length === 0 || selectedGroups.includes(ch.channel.group)) &&
          Object.values(ch.programs).some((programsArray) =>
            programsArray.some((program) => program.categories.includes(category)),
          ),
      ).length
    })
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      <Button onClick={() => window.location.reload()} variant="outline" size="sm" className="gap-1">
  ...
  const sidebar = (
    <SidebarContainer>
      <SidebarHeader>
        <SidebarSearch value={filterText} onChange={setFilterText} placeholder="Search channels..." />
      </SidebarHeader>
      <SidebarContent>
        <FilterSection
      ...
      <SidebarFooter>
        ...
        <div className="mt-2 text-muted-foreground text-xs text-center">
          Showing {filteredAndSortedChannels.length} of {moviesData?.channels.length || 0} channels
        </div>
      </SidebarFooter>
    </SidebarContainer>
  )
      ...
      <div className="p-4 pb-4">
        <div className="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          {filteredAndSortedChannels.map((channelData) => (
            <div key={channelData.channel.slug} className="flex flex-col bg-card shadow-sm border rounded-md h-full">
              {/* Channel header */}
              <div className="flex justify-between items-center px-3 py-2 border-b">
                ...
                <div className="ml-auto text-right">
                  <div className="font-medium text-base">{decodeHtml(channelData.channel.name)}</div>
                  {channelData.channel.lcn !== "N/A" && (
                    <div className="text-muted-foreground text-xs">Channel {channelData.channel.lcn}</div>
                  )}
                </div>
              </div>
                          ...
                          <table className="w-full min-w-full text-sm border-collapse table-auto">
                            <thead>
                              <tr className="border-b">
                                <th className="px-3 py-1 w-[100px] font-medium text-left">Time</th>
                                <th className="px-3 py-1 font-medium text-left">Title</th>
                              </tr>
                            </thead>
                            <tbody>
                              {programs.map((program, index) => (
                                <tr key={index} className="border-muted/20 border-b last:border-b-0">
                                  ...
                                  <td className="px-3 py-1">
                                    ...
                                    <div className="text-muted-foreground text-xs">{program.description}</div>
                                  </td>
                                </tr>
                              ))}
                            </tbody>
              ...
              <div className="flex mt-auto px-2 py-2 border-t">
                <Button variant="secondary" size="sm" className="flex-1 mr-2" onClick={navigateToNext24Hours}>
